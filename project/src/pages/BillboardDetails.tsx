...
const BillboardDetails: React.FC = () => {
  ...
  // Mock two-sided billboard data (kept as you had it)
  const [billboardSides] = useState([
    {
      ...
      price_per_day: 0, // will use billboard?.pricePerDay below when needed
      is_available: true,
  ...
  const coerceFeatures = (raw: any): string[] => {
    if (!raw) return [];
  ...
  const loadBillboard = async () => {
        ...
        if (data) {
          const mainImage =
            data.billboard_images?.find((img: any) => img.image_type === "main")
              ?.image_url ||
            (data.billboard_images && data.billboard_images.length > 0
    ...
    const mockBillboard = mockBillboards.find(
    ...
    if (mockBillboard) {
      const transformedBillboard = {
        ...
        features: coerceFeatures(mockBillboard.features),
      ...
      if (bookings && bookings.length > 0) {
        ...
        setBookingStatus({
          ...
          nextAvailable: new Date(
            new Date(latestBooking.end_date).getTime() + 24 * 60 * 60 * 1000
          ).toLocaleDateString(),
  ...
  const handleAddToCart = async (side: "A" | "B" | "BOTH" | "SINGLE") => {
    ...
    try {
      ...
      const chosenSide =
        billboardSides.find((s) => s.side_identifier === side) ||
        billboardSides[0];

      const pricePerDay =
        chosenSide?.price_per_day && chosenSide.price_per_day > 0
          ? chosenSide.price_per_day
          : billboard?.pricePerDay || 0;

      console.log("Starting Add to Cart...");
      // Now addToCart returns the refreshed cart session
      ...
      // Dispatch cartUpdated with the returned cart (if any),
      // otherwise fetch the cart and dispatch that.
      if (updatedCart) {
        window.dispatchEvent(
          new CustomEvent("cartUpdated", { detail: updatedCart })
        );
      } else {
        const newCart = await CartService.getUserCart(user.id);
        window.dispatchEvent(
          new CustomEvent("cartUpdated", { detail: newCart })
        );
      }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="grid grid-cols-4 gap-2">
            {[billboard.imageUrl, ...(billboard.additionalImages || [])].map(
              (img, index) => (
                <div
                  key={index}
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md h-fit">
          ...
          <div className="flex items-center mb-6">
            ...
            <span className="text-3xl font-bold text-blue-900 dark:text-blue-400">
              {billboard.pricePerDay.toLocaleString()}
            </span>
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center text-gray-700 dark:text-gray-300 mb-2">
              ...
              <span>
                {billboard.impressions.toLocaleString()}+ daily impressions
              </span>
            </div>
          </div>
            ...
            {bookingStatus?.isBooked ? (
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  This billboard is currently available for booking. Reserve
                  your dates now before someone else does!
                </p>
              </div>
            )}
          </div>
          ...
          <button className="w-full bg-white dark:bg-gray-700 border border-blue-900 dark:border-blue-400 text-blue-900 dark:text-blue-400 py-3 px-6 rounded-md font-semibold text-center hover:bg-blue-50 dark:hover:bg-gray-600 transition-colors flex items-center justify-center">
      ...
      <div className="mt-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
            ...
            <p className="text-gray-700 dark:text-gray-300">
              This premium billboard is strategically located in a high-traffic
              area with excellent visibility. The location offers exceptional
              exposure to both vehicular and pedestrian traffic.
            </p>
          </div>
        </div>
            ...
            <div className="space-y-4">
              {similarBillboards
                .filter(
                  (b) =>
                    b.id !== billboard.id &&
                    b.location.includes(billboard.location.split(",")[0])
                )
                .slice(0, 3)
                .map((b) => (
                  <Link
